...
impl Dataset {
    pub fn from_file(path: &str, tokenizer: &AkashaTokenizer, seq_len: usize) -> Self {
        ...
        // Tokenizing the full file in one encode() call blew up to a ~14GB
        // allocation (the tokenizers crate's working-set overhead is much
        // larger than the input text size). Truncate to a size that still
        // gives plenty of tokens (~15M from 50M chars) without exhausting RAM.
        ...
        println!("Tokenized {} chars into {} tokens", text.len(), tokens.len());
        Self { tokens, seq_len }
    }
