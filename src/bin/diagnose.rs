...
// Seeded (not thread_rng()) so CUDA-vs-Vulkan A/B runs see byte-identical
// "random" inputs/batches -- a prerequisite for the CUDA divergence
// investigation, where any difference must come from the backends'
// numerics, not from each run drawing different data. One shared,
// once-seeded generator (rather than re-seeding fresh per call) so
// sequential calls still advance through the sequence and don't all
// return the same values.
static DIAG_RNG: std::sync::OnceLock<std::sync::Mutex<rand::rngs::StdRng>> = std::sync::OnceLock::new();
...
// =====================================================================
fn check1_param_count(model: &AkashaModel) -> (bool, u64) {
    let total: u64 = model
        .trainable_params()
        .iter()
        .map(|(w, _)| w.size / 4)
...
// =====================================================================
// CHECK 2: gradient flow per layer
// =====================================================================
fn check2_grad_flow(ctx: Arc<WgpuContext>) -> bool {
    ...
    let model = AkashaModel::new(ctx.clone(), VOCAB_SIZE, DIM, seq_len, NUM_LAYERS, NUM_HEADS, &input_tokens);

    model.zero_grad();
    model.zero_transient_grads();
    model.cross_entropy.target_tokens.copy_from_cpu(&rand_u32_vec(seq_len as usize, VOCAB_SIZE));
    ...
    println!("CHECK 2: 1-step grad flow test (seq_len={seq_len}, dim={DIM}, layers={NUM_LAYERS}, heads={NUM_HEADS})");
    ...
    let vanishing = if layer_total_norms[0] > 1e-9 {
        ...
        if ratio > 1e3 {
            println!(
                ...
                layer_total_norms[0], NUM_LAYERS - 1, layer_total_norms[NUM_LAYERS - 1], ratio
            );
            true
        } else {
            false
...
// =====================================================================
// CHECK 3: HeadGather / HeadScatter correctness
// =====================================================================
fn check3_head_gather_scatter(ctx: Arc<WgpuContext>) -> bool {
    let seq_len = 4u32;
    let dim = 768u32;
    let head_dim = 64u32;
    ...
    // ---- round-trip identity: gather every head then scatter into a fresh buffer ----
    let reconstructed = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0.0f32; (seq_len * dim) as usize]));
    let mut g = ComputeGraph::new(ctx.clone());

    let mut head_bufs = Vec::with_capacity(num_heads as usize);
    for h in 0..num_heads {
        let head_offset = h * head_dim;
        let meta = Arc::new(Tensor::init_from_cpu(ctx.clone(), &[seq_len, dim, head_dim, head_offset]));
        let head_buf = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0.0f32; (seq_len * head_dim) as usize]));

        g.add_node(
            &shader_gather,
            &[
                TensorBind { binding: 0, tensor: &input, mode: TensorMode::Input },
                TensorBind { binding: 1, tensor: &head_buf, mode: TensorMode::Output },
                TensorBind { binding: 2, tensor: &meta, mode: TensorMode::Meta },
            ],
        ...
        g.add_node(
            &shader_scatter,
            &[
                TensorBind { binding: 0, tensor: &head_buf, mode: TensorMode::Input },
                TensorBind { binding: 1, tensor: &reconstructed, mode: TensorMode::Output },
                TensorBind { binding: 2, tensor: &meta, mode: TensorMode::Meta },
            ],
            [(head_dim + 15) / 16, (seq_len + 15) / 16, 1],
        );
    ...
    println!("CHECK 3: HeadGather/HeadScatter round-trip identity, max diff = {max_roundtrip_diff:.8} -> {}",
        if roundtrip_pass { "PASS" } else { "FAIL" });

    // ---- backward: f(x) = sum(HeadGather(x, head=0)); analytic grad via HeadScatter of all-ones ----
    let (_, meta0, head_offset0) = &head_bufs[0];
    let ones = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![1.0f32; (seq_len * head_dim) as usize]));
    let analytic_grad = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0.0f32; (seq_len * dim) as usize]));
    let mut g2 = ComputeGraph::new(ctx.clone());
    g2.add_node(
        &shader_scatter,
        &[
            TensorBind { binding: 0, tensor: &ones, mode: TensorMode::Input },
            TensorBind { binding: 1, tensor: &analytic_grad, mode: TensorMode::Output },
            TensorBind { binding: 2, tensor: meta0, mode: TensorMode::Meta },
        ],
        [(head_dim + 15) / 16, (seq_len + 15) / 16, 1],
    );
    g2.execute();
    ...
    // numerical grad of f(x) = sum(x[:, head_offset0 .. head_offset0+head_dim]) is exactly
    // 1.0 inside that column range and 0.0 elsewhere; verify via real finite differences
    // on a few sampled indices rather than asserting the closed form blindly.
    let eps = 1e-2f32;
    ...
    for &idx in &test_indices {
        ...
        let fp: f32 = if in_head {
            (0..head_dim).map(|d| xp[(row * dim + head_offset0 + d) as usize]).sum()
        } else {
            (0..head_dim).map(|d| input_data[(row * dim + head_offset0 + d) as usize]).sum()
        };
        let mut xm = input_data.clone();
        xm[idx] -= eps;
        let fm: f32 = if in_head {
            (0..head_dim).map(|d| xm[(row * dim + head_offset0 + d) as usize]).sum()
        } else {
            (0..head_dim).map(|d| input_data[(row * dim + head_offset0 + d) as usize]).sum()
        };
    ...
    println!("CHECK 3: HeadGather/HeadScatter backward, max numeric-vs-analytic diff = {max_grad_diff:.6} -> {}",
...
// =====================================================================
// CHECK 4: RMSNorm backward correctness
// =====================================================================
// f64 accumulation throughout -- the f32 version suffered catastrophic
// cancellation (summing ~6144 terms of magnitude ~1.7 each, then differencing
// two ~10000-magnitude sums to recover a ~1e-3-scale finite-difference signal
// blew past f32's ~7 significant digits).
fn cpu_rmsnorm_row_sum_f64(row: &[f32], w: &[f32], dim: usize) -> f64 {
    ...
    (0..dim).map(|d| (row[d] as f64) * rsqrt * (w[d] as f64)).sum()
}

fn cpu_rmsnorm_sum_f64(x: &[f32], w: &[f32], seq_len: usize, dim: usize) -> f64 {
    (0..seq_len).map(|i| cpu_rmsnorm_row_sum_f64(&x[i * dim..(i + 1) * dim], w, dim)).sum()
}

fn check4_rmsnorm_backward(ctx: Arc<WgpuContext>) -> bool {
    let seq_len = 8u32;
    let dim = 768u32;
    ...
    let grad_out = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![1.0f32; n])); // d(sum)/d(out) = 1
    let grad_in = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0.0f32; n]));

    let norm = RMSNorm::new(ctx.clone(), dim, seq_len, &w_data, &x_buf, &grad_out, &grad_in);
    norm.forward();
    norm.backward();
    ...
    // x only affects its own row -- evaluate that row alone (f64) to avoid
    // any cancellation from the other 7 rows' unrelated magnitude.
    ...
    println!("CHECK 4: RMSNorm backward, max numeric-vs-analytic diff = {max_diff:.6} -> {}",
        if pass { "PASS" } else { "FAIL" });
    pass
}

// =====================================================================
// CHECK 5: gradient accumulation correctness
// =====================================================================
fn check5_accumulation(ctx: Arc<WgpuContext>) -> bool {
    use akasha_core::config::{DIM, NUM_HEADS, NUM_LAYERS, VOCAB_SIZE};
    let seq_len = 16u32;

    let input_tokens = Arc::new(Tensor::init_from_cpu(ctx.clone(), &rand_u32_vec(seq_len as usize, VOCAB_SIZE)));
    let model = AkashaModel::new(ctx.clone(), VOCAB_SIZE, DIM, seq_len, NUM_LAYERS, NUM_HEADS, &input_tokens);
    ...
    let delta0: f32 = w_before.iter().zip(w_after_step0.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>().sqrt();
    ...
    let delta1: f32 = w_after_step0.iter().zip(w_after_step1.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>().sqrt();

    println!("CHECK 5: gradient accumulation");
    println!("  after step 0 (mid-cycle): weight delta norm = {delta0:.8}, grad_weight norm = {grad_after_step0:.6}");
...
// =====================================================================
// CHECK 6: AdamW weight decay grouping
// =====================================================================
fn check6_weight_decay_groups(model: &AkashaModel) -> bool {
    let params = model.trainable_params();
    let emb_in_group = params.iter().any(|(w, _)| Arc::ptr_eq(w, &model.embedding.table));
    let norm_in_group = params.iter().any(|(w, _)| Arc::ptr_eq(w, &model.final_norm.weight));
    ...
    println!("  most GPT-2-style training setups exempt 1D params (norms, embeddings, biases) from decay.");
    // Not a hard failure -- this is working as coded, just worth flagging per the user's ask.
    emb_in_group && norm_in_group
}

// =====================================================================
// CHECK 7: cross-entropy loss function correctness
// =====================================================================
fn check7_cross_entropy(ctx: Arc<WgpuContext>) -> bool {
    let vocab_size = 50257u32;
    let seq_len = 4u32;

    let logits = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0.0f32; (vocab_size * seq_len) as usize]));
    let grad_logits = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0.0f32; (vocab_size * seq_len) as usize]));
    ...
    let pass = diff < 0.01;
    println!("CHECK 7: cross-entropy on all-zero logits, expected ln({vocab_size}) = {expected:.4}, got = {got:.4}, diff = {diff:.4} -> {}",
        if pass { "PASS" } else { "FAIL" });
    pass
}

// =====================================================================
// CHECK 8: single-layer memorization test (the most important check)
// =====================================================================
fn check8_memorization(ctx: Arc<WgpuContext>) -> bool {
    // Uses whichever backend select_backend() picked (CUDA if --features cuda
    // and a CUDA device is available, else Vulkan) -- this needs to run on
    // both backends in turn (set/unset --features cuda) to A/B compare, so it
    // must NOT force one backend internally.
...
fn check8_run(ctx: Arc<WgpuContext>, lr: f32, use_clip: bool) -> bool {
    let dim = 64u32;
    let num_heads = 4u32;
    ...
    let input_tokens = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0u32; seq_len as usize]));
    let model = AkashaModel::new(ctx.clone(), vocab_size, dim, seq_len, num_layers, num_heads, &input_tokens);

    // fixed batch, reused every iteration
    ...
    model.cross_entropy.set_grad_scale(1.0 / (seq_len as f32 * batch_size as f32));

    println!("CHECK 8: single-layer memorization test (dim={dim}, heads={num_heads}, layers={num_layers}, vocab={vocab_size}, seq_len={seq_len}, batch={batch_size}, lr={lr})");
    ...
    for step in 0..600usize {
        ...
        for i in 0..batch_size {
            ...
            model.cross_entropy.target_tokens.copy_from_cpu(&targets[window]);
            model.zero_transient_grads();
            model.forward_fused();
            if step == 0 && i == 0 {
                // Forward-intermediate fingerprint, captured BEFORE backward
                // runs. The aggregate loss matching exactly between backends
                // doesn't guarantee every intermediate activation matches --
                // it's one scalar aggregate, and small per-element forward
                // differences could exist without moving the displayed loss.
                fn norm_f32(t: &Arc<Tensor>) -> f64 {
                    let data: Vec<f32> = t.to_cpu();
        ...
        if step == 0 {
            // Per-tensor breakdown to localize which backward op first
            // diverges between backends (the aggregate grad_norm already
            // differs ~26% at step 0, so the bug is in backward, not AdamW).
            fn norm(t: &Arc<Tensor>) -> f64 {
                let data: Vec<f32> = t.to_cpu();
                data.iter().map(|&x| (x as f64) * (x as f64)).sum::<f64>().sqrt()
            }
            println!("  [fp-detail] lm_head.grad_input(=dY into final_norm.backward)={:.8}", norm(&model.lm_head.grad_input));
            for (i, layer) in model.layers.iter().enumerate() {
                println!("  [fp-detail] layer {i}: Q={:.6} K={:.6} V={:.6} O={:.6} FFNup={:.6} FFNdown={:.6} Norm1={:.6} Norm2={:.6}",
                    norm(&layer.q_proj.grad_weight), norm(&layer.k_proj.grad_weight), norm(&layer.v_proj.grad_weight),
                    norm(&layer.out_proj.grad_weight), norm(&layer.ffn_up.grad_weight), norm(&layer.ffn_down.grad_weight),
                    norm(&layer.norm_1.grad_weight), norm(&layer.norm_2.grad_weight));
            }
            println!("  [fp-detail] embedding={:.6} final_norm={:.6} lm_head={:.6}",
                norm(&model.embedding.grad_table), norm(&model.final_norm.grad_weight), norm(&model.lm_head.grad_weight));
        }
        if step < 10 {
            // Fine-grained per-step fingerprint for CUDA-vs-Vulkan A/B: grad
            // norm (post-backward, pre-clip), and AdamW's m/v state (post-step)
            // summed across all trainable params. Run identically-seeded on
            // both backends and diff these to find the exact step + buffer
            // where they first measurably split.
    ...
    println!("CHECK 8: final loss = {final_loss:.4} -> {}", if pass { "PASS" } else { "FAIL" });
    if !pass {
        println!("  RED FLAG: tiny single-layer model could not memorize a fixed batch in 200 steps.");
        println!("  This points to a bug in the training loop itself (optimizer, backward, or loss),");
...
fn main() {
    ...
    // Checks 1-7 each construct their own full-size model (plus main()'s
    // own `full_model` for checks 1/6), and several stay alive at once --
    // on Vulkan that alone exceeds the RTX4050's 6GB VRAM. Allow isolating
    // CHECK 8 (the only one needed for the CUDA-vs-Vulkan A/B comparison)
    // to avoid that unrelated OOM.
    if std::env::var("DIAGNOSE_ONLY_CHECK8").is_ok() {
        let c8_pass = check8_memorization(ctx.clone());
        println!("CHECK 8 (memorization): {}", if c8_pass { "PASS" } else { "FAIL" });
        return;
    }
    ...
    let input_tokens = Arc::new(Tensor::init_from_cpu(ctx.clone(), &vec![0u32; SEQ_LEN as usize]));
    let full_model = AkashaModel::new(ctx.clone(), VOCAB_SIZE, DIM, SEQ_LEN, NUM_LAYERS, NUM_HEADS, &input_tokens);
    ...
    println!("\n================= SUMMARY =================");
    println!("CHECK 1 (param count):          {} -- {} parameters", if c1_pass { "PASS" } else { "FAIL" }, c1_count);
    println!("CHECK 2 (gradient flow):        {}", if c2_pass { "PASS" } else { "FAIL" });
    println!("CHECK 3 (HeadGather/Scatter):   {}", if c3_pass { "PASS" } else { "FAIL" });
    println!("CHECK 4 (RMSNorm backward):     {}", if c4_pass { "PASS" } else { "FAIL" });
    println!("CHECK 5 (grad accumulation):    {}", if c5_pass { "PASS" } else { "FAIL" });
    println!("CHECK 6 (weight decay groups):  {} (advisory)", if c6_pass { "PASS" } else { "FAIL" });
    println!("CHECK 7 (loss function):        {}", if c7_pass { "PASS" } else { "FAIL" });
    println!("CHECK 8 (memorization):          {}", if c8_pass { "PASS" } else { "FAIL" });
}
